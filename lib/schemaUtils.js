...
const async = require('async'),
  ...
  postMethods = require('./clearAgMethods.js'),
  SCHEMA_FORMATS = {
...
/**
   * Adds Postman Collection Items using extensions.
   * Each operation from OpenAPI spec object is mapped to a item-group (Folder)
   *
   * @param {object} spec - openAPI spec object
   * @param {object} generatedStore - the store that holds the generated collection. Modified in-place
   * @param {object} components - components defined in the OAS spec. These are used to
   ...
   * @returns {object} returns an object containing objects of tags and their requests
   */
  addCollectionItemsUsingExtensions: function(spec, generatedStore, components, options, schemaCache) {
    ...
    _.forEach(paths, (currentPathObject, path) => {
      ...
      _.forEach(pathMethods, (pathMethod) => {
        ...
        // get folder path extension
        if (operationItem.hasOwnProperty('postman_config')) {
          folderPath = operationItem['postman_config']['folder_path']
        ...
        // add the request which has no folder path
        ...
        // determine folder path and insert request
        else {
          parentFolder = folderPath[0]
          if (!extensionFolders.hasOwnProperty(parentFolder)) {      
            extensionFolders[parentFolder] = {
              description: '',
              requests: [],
              children: {}
            }
          }
          rootPath = extensionFolders[parentFolder]
          for(i = 1; i < folderPath.length; i++) {
            if (!rootPath.children.hasOwnProperty(folderPath[i])) {
              rootPath.children[folderPath[i]] = {
                description: '',
                requests: [],
                children: {}
              }
            }
            rootPath = rootPath.children[folderPath[i]]
          }
          rootPath.requests.push({
            name: summary,
            method: pathMethod,
            path: path,
            properties: operationItem,
    ...
    // Generate item groups from folder structure
    this.createItemGroups(spec, extensionFolders, generatedStore, components, options, schemaCache, variableStore)
  ...
  /**
   * Adds Postman Collection Item groups using folder structure.
   * Each operation from OpenAPI spec object is mapped to a item-group (Folder)
   *
   * @param {object} spec - openAPI spec object
   * @param {object} extensionFolders - folder structure generated from open api spec
   * @param {object} generatedStore - the store that holds the generated collection. Modified in-place
   * @param {object} components - components defined in the OAS spec. These are used to
   *  resolve references while generating params.
   * @param {object} options - a standard list of options that's globally passed around. Check options.js for more.
   * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
   * @param {object} variableStore - object storing variables generated from open api spec
   */
  createItemGroups: function(spec, extensionFolders, generatedStore, components, options, schemaCache, variableStore) {
    // Add all folders created from extensions and corresponding operations
    _.forEachRight(extensionFolders, (extFolder, extName) => {
      var itemGroup = new sdk.ItemGroup({
        name: extName,
        description: extFolder.description
      });

      if (!_.isEmpty(extFolder.children)) {
        this.createChildGroup(itemGroup, extFolder, spec, components, options, schemaCache, variableStore)
      }
      
      _.forEach(extFolder.requests, (request) => {
        itemGroup.items.add(this.convertRequestToItem(spec, request, components, options, schemaCache, variableStore));
      });
  ...
  createChildGroup: function(topLevelGroup, topLevelFolder, spec, components, options, schemaCache, variableStore) {
    _.forEach(topLevelFolder.children, (childFolder, childFolderName) => {
      var childGroup = new sdk.ItemGroup({
        name: childFolderName,
        description: childFolder.description
      })

      if (!_.isEmpty(childFolder.children)) {
        this.createChildGroup(childGroup, childFolder, spec, components, options, schemaCache, variableStore)
      }

      topLevelGroup.items.add(childGroup)
      
      _.forEach(childFolder.requests, (request) => {
        childGroup.items.add(this.convertRequestToItem(spec, request, components, options, schemaCache, variableStore));
      });
    });
  },
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  convertToPmQueryParameters: function(param, requestType, components, options, schemaCache) {
    ...
    if (param.hasOwnProperty('schema')) {
      ...
      // replace params with PostMan Variables as default
      // only replaces params that are defaulting to <type>, e.g <string> or <uuid>
      var replaceParams = ['user_id', 'account_id', 'zone_map_id', 'field_id', 'growth_id']
      if (replaceParams.includes(param.name)) {
  ...
  */
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore) {
    ...
    // add post script/test
    if(operation.hasOwnProperty('postman_config')) {
      var storeVar = operation['postman_config']['var_name']
      var parse_func = operation['postman_config']['parse_func']
      var itemEvent = postMethods.getPostScript(storeVar, parse_func)
    }
    // if postman_config is not found, only set post test for HTTP 200
    else {
      var itemEvent = postMethods.getPostScript("", "http_200")
    }
